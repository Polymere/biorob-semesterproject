...
class NewEvolutionController():
	...
	def __init__(self, ev_config):
		...
		self.__dict__.update(ev_config["controller_args"])
		...
		if self.optimizer_name=="NSGAII":
			self.union_pop=True
			self.opti=NSGAIIOptimizer(ev_config["optimizer_args"])
		elif self.optimizer_name=="GA":
			self.union_pop=False
			self.opti=GAOptimizer()
		elif self.optimizer_name=="PSO":
			self.union_pop=False
			self.opti=PSOptimizer()
		else:
	...
	def set_init_pop(self):
		...
		if self.init_pop_mode=="rand":
			...
			self.current_pop=pop_df
			self.current_pop=self.opti.get_next_gen(select_ids)
	...
	def init_pop(self):
		...
		self.nb_gen+=1
		self.childrens=self.opti.get_next_gen(eval_pop)
	...
	def evolve(self):
		...
		while not self.is_stop():
			...
			if self.union_pop:
				candidates=pd.concat([eval_df,self.current_pop],axis=0)
			else:
				candidates=eval_df
			self.current_pop=candidates # 
			self.childrens=self.opti.get_next_gen(candidates) # drop fitnesses and cr_dist for check_bound
	...
	def eval_pop(self,population,verbose=False):
		...
		df=pd.concat([population,scores],axis=1)
		evaluated=df[df.columns.drop(df.filter(like="Unnamed").columns)]
		evaluated.dropna(axis="columns",inplace=True)
		saved_df=evaluated
		saved_df["gen"]=self.nb_gen
		saved_df.to_csv(os.path.join(self.trial_dir,"gen"+str(self.nb_gen)+".csv"))
		evaluated=evaluated.add(evaluated.fit_stable,axis='index')
		evaluated.drop("fit_stable",axis=1,inplace=True)
...
class NewCppEvolutionController(EvolutionController):
	def __init__(self,config_file):
		params=yaml.load(open(config_file, 'r'))
...
if __name__ == '__main__':
	...
	if model=="cpp":
		...
		config_file="../../data/references/ev_NSGAII_template.yaml"
		assert_file_exists(config_file, should_exist=True)
